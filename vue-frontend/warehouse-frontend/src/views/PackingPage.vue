...
export default {
  ...
  methods: {
    ...
    async scanAsset() {
      // Validating input And prevent calling API
      if(!this.assetId){
        return;
      }
      ...
      try {
        ...
        if (transaction.response_type === "OK") {
          ...
          //   sessionStorage.setItem('inventoryList', JSON.stringify(this.inventoryList));
          // }
          this.updateInventoryList(data.parent_data.child_assets, transaction.assetId)
    ...
    updateInventoryList(assets, assetId) {
      const index = assets.findIndex(asset => asset.Name === assetId);
      if (index !== -1) {
        const [foundAsset] = assets.splice(index, 1);
        assets.unshift(foundAsset);
        this.inventoryList = assets
        sessionStorage.setItem('inventoryList', JSON.stringify(this.inventoryList));
      }
    },
    async forceAsset(forcedAsset) {
      ...
      try {
        ...
        if (forcedAsset.response_type === "OK") {
          ...
          //   sessionStorage.setItem('inventoryList', JSON.stringify(this.inventoryList));
          // }
          this.updateInventoryList(data.parent_data.child_assets, forcedAsset.assetId)
